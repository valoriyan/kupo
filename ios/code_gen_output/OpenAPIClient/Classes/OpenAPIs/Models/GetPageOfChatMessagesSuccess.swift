//
// GetPageOfChatMessagesSuccess.swift
//
// Generated by openapi-generator
// https://openapi-generator.tech
//

import Foundation
#if canImport(AnyCodable)
import AnyCodable
#endif

public struct GetPageOfChatMessagesSuccess: Codable, JSONEncodable, Hashable {

    public var chatMessages: [RenderableChatMessage]
    public var previousPageCursor: String?
    public var nextPageCursor: String?

    public init(chatMessages: [RenderableChatMessage], previousPageCursor: String? = nil, nextPageCursor: String? = nil) {
        self.chatMessages = chatMessages
        self.previousPageCursor = previousPageCursor
        self.nextPageCursor = nextPageCursor
    }

    public enum CodingKeys: String, CodingKey, CaseIterable {
        case chatMessages
        case previousPageCursor
        case nextPageCursor
    }

    // Encodable protocol methods

    public func encode(to encoder: Encoder) throws {
        var container = encoder.container(keyedBy: CodingKeys.self)
        try container.encode(chatMessages, forKey: .chatMessages)
        try container.encodeIfPresent(previousPageCursor, forKey: .previousPageCursor)
        try container.encodeIfPresent(nextPageCursor, forKey: .nextPageCursor)
    }
}

