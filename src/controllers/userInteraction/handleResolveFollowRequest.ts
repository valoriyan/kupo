...
export async function handleResolveFollowRequest({
  ...
  if (decision === FollowRequestDecision.accept) {
    const approvePendingFollowResponse = await controller.databaseService.tableNameToServicesMap.userFollowsTableService.approvePendingFollow({
        controller,
    ...
    const deleteUserFollowResponse = await controller.databaseService.tableNameToServicesMap.userFollowsTableService.deleteUserFollow({
        controller,
  ...
  return Failure({
    ...
    additionalErrorInformation:
      "Unknown decision type at handleResolveFollowRequest",
