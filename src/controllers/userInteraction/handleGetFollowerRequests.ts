...
import { collectMappedResponses, EitherType, ErrorReasonTypes, HTTPResponse, InternalServiceResponse, Success } from "../../utilities/monads";
...
export async function handleGetFollowerRequests({
  ...
  const getUserIdsFollowingUserIdResponse = await controller.databaseService.tableNameToServicesMap.userFollowsTableService.getUserIdsFollowingUserId({controller, userIdBeingFollowed: clientUserId, areFollowsPending: true});
  ...
  if (mappedConstructRenderableUserFromPartsByUserIdResponses.type === EitherType.failure) {
    return mappedConstructRenderableUserFromPartsByUserIdResponses;
  }
  const { success: renderableUsers } = mappedConstructRenderableUserFromPartsByUserIdResponses;
