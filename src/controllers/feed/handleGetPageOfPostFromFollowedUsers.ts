...
import { RenderablePost, UnrenderablePostWithoutElementsOrHashtags } from "../post/models";
...
> {
  ...
  if (error) return error;
    ...
    const userIdsBeingFollowed: string[] = await controller.databaseService.tableNameToServicesMap.userFollowsTableService.getUserIdsFollowedByUserId({userIdDoingFollowing: clientUserId});

    const unrenderablePostsWithoutElementsOrHashtags = await controller.databaseService.tableNameToServicesMap.postsTableService.getPostsByCreatorUserIds({creatorUserIds: userIdsBeingFollowed});

    const filteredUnrenderablePostsWithoutElements: UnrenderablePostWithoutElementsOrHashtags[] = getPageOfPosts({
      unfilteredUnrenderablePostsWithoutElementsOrHashtags: unrenderablePostsWithoutElementsOrHashtags,
